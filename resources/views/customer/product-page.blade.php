  ...
  <script>
    ...
    async function fetchPagination(totalPage) {
      ...
      if (totalPage > 1) {
        ...
        var pageNumbersHTML = `<li id="page-previous" class:"pagination flex"><a class="cursor-pointer" onClick="loadPage(0, ${totalPage})"><<</a></li>`;
        for (var i = 1; i <= totalPage; i++) {
          if (window.current_page == i) {
            pageNumbersHTML += `<li id="page-${i}" class="pagination bg-[#518581] text-white flex"><a class="cursor-pointer" onClick="loadPage(${i}, ${totalPage})">${i}</a></li>`;
          } else {
            pageNumbersHTML += `<li id="page-${i}" class:"pagination flex"><a class="cursor-pointer" onClick="loadPage(${i}, ${totalPage})">${i}</a></li>`;
          }
        }
        pageNumbersHTML += `<li id="page-next" class:"pagination flex"><a class="cursor-pointer" onClick="loadPage(${totalPage + 1}, ${totalPage})">>></a></li>`;
    ...
    function handleFilterCategories(dmucID) {
      ...
      clickedButton.setAttribute('class', 'category-button h-full w-full text-2xl py-3 text-center bg-[#518581] text-white font-medium shadow-[4px_4px_4px_rgba(0,0,0,0.25)]')
      if (dmucID != 'all') {
        dmuc = dmucID;
      } else {
        dmuc = '';
...
<body>
  ...
  <main>
      ...
      <div class="swiper-wrapper">
        <div class="swiper-slide slider-1">
          <div class="w-full h-full flex items-start justify-center ml-5 lg:ml-[132px] lg:gap-y-5 gap-y-3 flex-col">
            <span class="font-medium text-[10px] lg:text-sm bg-[#518581] rounded-[16px] lg:px-[16px] lg:py-[8px] px-[10px] py-[6px]">Discount</span>
        ...
        <div class="swiper-slide slider-2">
          <div class="w-full h-full flex items-start justify-center ml-5 lg:ml-[132px] lg:gap-y-5 gap-y-3 flex-col">
            <span class="font-mediup text-[10px] lg:text-sm bg-[#518581] rounded-[16px] lg:px-[16px] lg:py-[8px] px-[10px] py-[6px]">Discount</span>
            <h2 class="text-base lg:text-[44px] font-bold">
              Ramadhan Sale Offer
            </h2>
            <p class="lg:text-[24px] text-[12px] font-medium">
    ...
    <div class="container shadow-[0px_4px_120px_rgba(175,173,181,0.25)] w-full lg:w-[90%] bg-white my-[30px] lg:mt-[40px] lg:mb-[50px] px-[30px] py-[15px] flex items-center justify-center">
      <svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg" class=" ">
        <path d="M15.9645 28C22.9601 28 28.6312 22.3289 28.6312 15.3333C28.6312 8.33769 22.9601 2.66663 15.9645 2.66663C8.96891 2.66663 3.29785 8.33769 3.29785 15.3333C3.29785 22.3289 8.96891 28 15.9645 28Z" stroke="#AFADB5" strokeWidth="{2}" strokeLinecap="round" strokeLinejoin="round" />
        <path d="M29.9645 29.3333L27.2979 26.6666" stroke="#AFADB5" strokeWidth="{2}" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
      <input id="TimKiem" onkeyup="searchEnter(event)" type="text" class="outline-none px-[8px] lg:px-[18px] w-full h-[38px] lg:h-[66px] text-sm lg:text-base" placeholder="Nhập từ khóa tìm kiếm" />
      <button onclick="clearKeyword()">
        <svg width="25" height="32" class="dark:text-white mr-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="#AFADB5" viewBox="0 0 20 20">
          <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 11.793a1 1 0 1 1-1.414 1.414L10 11.414l-2.293 2.293a1 1 0 0 1-1.414-1.414L8.586 10 6.293 7.707a1 1 0 0 1 1.414-1.414L10 8.586l2.293-2.293a1 1 0 0 1 1.414 1.414L11.414 10l2.293 2.293Z" />
        </svg>
      </button>
      <button onclick="searchProduct()" class="text-white bg-[#518581] w-[169px] h-[38px] lg:h-[66px] lg:px-[33px] lg:py-[18px] px-[20px] py-[6px] text-sm lg:text-[18px] font-bold">
        Tìm kiếm
      </button>
    </div>
    ...
    <div class="lg:hidden heading-category flex items-center justify-start gap-x-[10px] p-5">
      ...
      <span class="w-[41px] h-[26px] rounded-3xl font-bold text-[12px] text-[#518581] px-4 py-[4.5px] bg-[#F9F9F9] flex items-center justify-center">184</span>
    </div>
    ...
    <div class="flex products-wrap container items-baseline justify-center lg:gap-x-[62px]">
      <div class="products-category flex items-center justify-center flex-col max-w-[296px] gap-y-[9px] gap-y-[9px]">
        <div class="hidden lg:flex heading-category items-center justify-center gap-x-[10px]">
          ...
          <span class="lg:w-[60px] lg:h-[30px] w-[41px] h-[26px] rounded-3xl font-bold text-[12px] lg:text-lg text-[#518581] px-4 py-[4.5px] bg-[#F9F9F9] flex items-center justify-center">184</span>
        </div>
        <span class="hidden lg:flex items-center justify-center gap-x-4">
          <svg width="27" height="26" viewBox="0 0 27 26" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-[31px] h-[31px]">
            <path fill-rule="evenodd" clip-rule="evenodd" d="M14.1974 0.654419L26.5199 11.8919L25.1443 13.2675L23.1874 11.4928V24.1369L22.2186 25.1057H16.4061L15.4374 24.1369V17.3557H11.5624V24.1369L10.5936 25.1057H4.78114L3.81239 24.1369V11.5083L1.87489 13.2675L0.499268 11.8919L12.8024 0.654419H14.1974ZM5.74989 9.74711V23.1682H9.62489V16.3869L10.5936 15.4182H16.4061L17.3749 16.3869V23.1682H21.2499V9.73548L13.4999 2.70817L5.74989 9.74711Z" fill="black" />
          </svg>
        ...
        <div class="hidden lg:flex items-center justify-center flex-col gap-y-[9px]">
          <div class="text-center w-[296px] h-[54px] text-white bg-[#518581] py-3 font-bold text-2xl leading-none flex items-center justify-center gap-x-[10px]">
            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fill-rule="evenodd" clip-rule="evenodd" d="M28.125 3.75V6.88125L18.75 15.8044V26.25H11.25V15.8044L1.875 6.87937V3.75H28.125ZM13.125 15V24.375H16.875V15L26.25 6.075V5.625H3.75V6.075L13.125 15Z" fill="#F9F9F9" />
            </svg>
          ...
          <label for="5-10" class="w-full select-none">
            <div class="h-full w-full bg-white font-light text-2xl py-3 pl-[35px] bg-[#F9F9F9] active:shadow-[4px_4px_4px_rgba(0,0,0,0.25)] flex items-center gap-x-[10px]">
              <input onclick="handlePriceChange(this,'under10')" type="radio" name="price" id="under10" class="w-8 h-8" />
              <span>Dưới 10 triệu</span>
            </div>
          </label>
          <label for="10-20" class="w-full select-none">
            <div class="h-full w-full bg-white font-light text-2xl py-3 pl-[35px] bg-[#F9F9F9] active:shadow-[4px_4px_4px_rgba(0,0,0,0.25)] flex items-center gap-x-[10px]">
              <input onclick="handlePriceChange(this,'10-20')" type="radio" name="price" id="10-20" class="w-8 h-8" />
              <span>Từ 10 - 20 triệu</span>
            </div>
          </label>
          <label for="over20" class="w-full select-none">
            <div class="h-full w-full bg-white font-light text-2xl py-3 pl-[35px] bg-[#F9F9F9] active:shadow-[4px_4px_4px_rgba(0,0,0,0.25)] flex items-center gap-x-[10px]">
              <input onclick="handlePriceChange(this,'over20')" type="radio" name="price" id="over20" class="w-8 h-8" />
      ...
      <div>
        <div class="lg:flex-1">
          ...
          <div id="data-container" class="lg:mt-[76px] grid grid-cols-2 md:grid-cols-3 md:gap-x-[25px] lg:gap-y-[45px] gap-x-3 gap-y-[15px]">
            <!-- Product 1 -->
